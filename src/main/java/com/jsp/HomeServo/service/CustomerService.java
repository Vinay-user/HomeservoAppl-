...
//			responseStructure.setData(customer1);
			com.jsp.HomeServo.duplicate.Customer customer2=(com.jsp.HomeServo.duplicate.Customer)this.modelMapper.map(customer, com.jsp.HomeServo.duplicate.Customer.class);
			responseStructure.setData(customer2);
			responseStructure.setMessage("customer find successfully");
			responseStructure.setStatus(HttpStatus.FOUND.value());

			return new ResponseEntity<ResponseStructure<com.jsp.HomeServo.duplicate.Customer>>(responseStructure, HttpStatus.FOUND);
		} else {
			throw new NoSuchElementFoundInCustomer("please check the id in Customer");
		}
